...
read.table("../Output/freqPatTsInclDay0-threshold.csv",sep=",",header=TRUE,row.names=1)->freqPatTsv2
...
for (patrow in 1:length(freqPatTsv2[,1])){
freqPatTsRandomSyn[patrow,sample(Asites)]<-freqPatTsv2Syn[patrow,Asites]
freqPatTsRandomSyn[patrow,sample(Csites)]<-freqPatTsv2Syn[patrow,Csites]
freqPatTsRandomSyn[patrow,sample(Gsites)]<-freqPatTsv2Syn[patrow,Gsites]
freqPatTsRandomSyn[patrow,sample(Tsites)]<-freqPatTsv2Syn[patrow,Tsites]
}
...
colMeansSyn<-apply(freqPatTsv2Syn, 2 , mean, na.rm=TRUE)
par(mfrow=c(1,1))
...
```{r}
freqPatTsv2NonSyn<-freqPatTsv2[,OverviewDF$TypeOfSite=="nonsyn"]
...
for (j in 1:20){
    if (j %% 10 == 0 ) print (j)
freqPatTsRandomNonSyn<-freqPatTsv2NonSyn

for (patrow in 1:length(freqPatTsv2[,1])){
freqPatTsRandomNonSyn[patrow,sample(AsitesNonSyn)]<-freqPatTsv2NonSyn[patrow,AsitesNonSyn]
freqPatTsRandomNonSyn[patrow,sample(CsitesNonSyn)]<-freqPatTsv2NonSyn[patrow,CsitesNonSyn]
freqPatTsRandomNonSyn[patrow,sample(GsitesNonSyn)]<-freqPatTsv2NonSyn[patrow,GsitesNonSyn]
freqPatTsRandomNonSyn[patrow,sample(TsitesNonSyn)]<-freqPatTsv2NonSyn[patrow,TsitesNonSyn]
}
...
Show the distribution of variances form the randomized data and compare with the value of the ofiginal dataset. It looks very different!
```{r}
hist(ListOfVarsNonSyn,breaks=20, xlim=c(2*10^-6,4*10^-5), main="Histogram of NONSYN variances vs observed variance")
abline(v=var(apply(freqPatTsv2NonSyn, 2 , mean, na.rm=TRUE)),col=2,lwd=2)

```
Show the distribution of the means for one randome dataset and the original data. One can see that the randomized data have much fewer 0s and fewer high values. 
```{r}
colMeansRandomNonSyn<-apply(freqPatTsRandomNonSyn, 2 , mean, na.rm=TRUE)
colMeansNonSyn<-apply(freqPatTsv2NonSyn, 2 , mean, na.rm=TRUE)
par(mfrow=c(1,1))
hist(apply(freqPatTsv2NonSyn, 2 , mean, na.rm=TRUE),breaks=seq(0,0.2,by=0.001),xlim=c(0,0.04),
     #ylim=c(0,40),
     col=rgb(1,0,0,0.5),main="Histogram of randomized (blue) vs observed (red) mean frequencies")
...
freqPatTsv2Stop<-freqPatTsv2[,OverviewDF$TypeOfSite=="stop"]
colMeansStop<-apply(freqPatTsv2Stop, 2 , mean, na.rm=TRUE)
...
hist(ListOfNumSites0Freq,xlim=c(10,50))
