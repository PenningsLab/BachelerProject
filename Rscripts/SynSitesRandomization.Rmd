...
#read.table("../Output/freqPatTsInclDay0-version2.csv",sep=",",header=TRUE,row.names=1)->freqPatTs #replacing the ts with the new version2
}
...
perc<-0.10
npatout<-ceiling(nrow(OverviewDFSyn)*perc)
OverviewDFSynOrdered<-OverviewDFSyn[order(OverviewDFSyn$colMeansTs),]
OverviewDFSynTail<-OverviewDFSynOrdered[1:(nrow(OverviewDFSynOrdered)-npatout),]
#OverviewDFSynTail<-tail(OverviewDFSyn[order(OverviewDFSyn$colMeansTs),],n=-ceiling(nrow(OverviewDFSyn)*perc))
...
#tail version: remove the 5% upper highest syn positions (=outliers)
...
pdf("../Output/Distribution_of_Variances_top5percentout.pdf",width = 13, height = 10)
hist(ListOfVars,breaks=20,xlim=c(.9*10^-5,10*10^-5), main="Histogram of variances vs observed variance (5%)")
...
pdf("../Output/Distribution_of_Means_top10percentout.pdf",width = 13, height = 10)
par(mfrow=c(1,1))
hist(colMeansSyn,breaks=seq(0,0.2,by=0.001),xlim=c(0,0.04),
     ylim=c(0,40),
     col=rgb(1,0,0,0.5),main="Histogram of mean frequencies 10%")
