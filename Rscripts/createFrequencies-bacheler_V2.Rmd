...
#option 1: considere positions seperately  (traditional manner)
 ...
 #print(paste('j',j))
		  ...
		  if(length(grep(FALSE,patfasta[day0sequences,j]==WT))>0){  # if the nucleotide of the first sequenceS is different from the consensus, then skip that patient 
		    ...
		    # match(j,seq(1,982,by=3)); match(j,seq(2,983,by=3)); match(j,seq(3,984,by=3)),

		     #if(match(j,seq(1,982,by=3))) {# first position
...
#option 2: consider the codon as such: (Marion idea)
	for (h in 1:328){  # for each codon 
	  ...
	  if(FALSE %in% names(table(consensusB[codonposition]==t(patfasta[day0sequences,codonposition])))){
	  #if(FALSE %in% names(table(consensusB[codonposition]==(apply(patfasta[day0sequences,codonposition], 1, paste))))){ # if there is one FALSE, then skip  #### problem with days0sequences
	      ...
	      freqPatTs_verion3[i,(((h-1)*3+1):(h*3))[3]]<-'NA'
	        ...
	        good2sequences<-which(apply(consensusB[codonposition]==(apply(patfasta[,codonposition], 1, paste)),2,sum) >1)   ## number of changes in the codon is 0 or 1 
