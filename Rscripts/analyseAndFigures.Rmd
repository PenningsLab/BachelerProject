...
* Transition function*
```{r}
#PSP Nov 11 2015 I moved the transition function to the baseRscript.R file
...
PSP Nov 11 2015, I moved the typeofsitefunction to baseRscript
...
#PSP Nov 11 2015 I included colMeansTsv2,colMeansTsv3 in OverviewDF and OverviewDFOrderedByFreq (previously x and newdata)
#version2 and version3: remove when not useful
#xv23<-data.frame(num=1:984,colMeansTs,colMeansTsv2,colMeansTsv3,TypeOfSite,lowerConf,upperConf)
#xv23$color<-""
#for (i in 1:984){
#    if (xv23$TypeOfSite[i]=="syn") xv23$color[i] = "green"
#    if (xv23$TypeOfSite[i]=="nonsyn") xv23$color[i] = "red"
#    if (xv23$TypeOfSite[i]=="stop") xv23$color[i] = "black"
#    if (xv23$TypeOfSite[i]=="res") xv23$color[i] = "purple"
#newdatav2 <- xv23[order(colMeansTsv2),] 
#newdatav3 <- xv23[order(colMeansTsv3),] 
#    }
...
plot(PROdatav2$colMeansTsv2+0.001,log="y",ylim=c(0.001,0.15),cex=2, pch = "|",col=PROdatav2$color)
dev.off()
...
plot(PROdatav3$colMeansTsv3+0.001,log="y",ylim=c(0.001,0.15),cex=2, pch = "|",col=PROdatav3$color)
dev.off()
```
...
A plot for all mutations does nt work because tv dominates the coloring. 
...
plot(RTdatav2$colMeansTsv2+0.001,log="y",ylim=c(0.001,0.15),cex=1, pch = "|",col=RTdatav2$color)
dev.off()
...
plot(RTdatav3$colMeansTsv3+0.001,log="y",ylim=c(0.001,0.15),cex=1, pch = "|",col=RTdatav3$color)
dev.off()
...
PROdataNonSynv2<-PROdatav2[PROdatav2$TypeOfSite=="nonsyn",]
plot(PROdataNonSynv2$num[order(PROdataNonSynv2$num)],PROdataNonSynv2$colMeansTsv2[order(PROdataNonSynv2$num)]+0.001,col=PROdataNonSynv2$color[order(PROdataNonSynv2$num)],pch=16,log="y",type="b")
#Add moving mean
for (i in 20:280){
    range = (i-20):(i+20)
    points(i, median(PROdataNonSynv2$colMeansTsv2[PROdataNonSynv2$num%in%range]), col="green",pch = 11, cex=3)
}
dev.off()

#VERSION 3
pdf("../Output/PR-nonsyn-version3.pdf",width = 13, height = 10)
PROdataNonSynv3<-PROdatav3[PROdatav3$TypeOfSite=="nonsyn",]
plot(PROdataNonSynv3$num[order(PROdataNonSynv3$num)],PROdataNonSynv3$colMeansTsv3[order(PROdataNonSynv3$num)]+0.001,col=PROdataNonSynv3$color[order(PROdataNonSynv3$num)],pch=16,log="y",type="b")
#Add moving mean
for (i in 20:280){
    range = (i-20):(i+20)
...
for (i in 20:280){
  ...
  points(i, median(PROdataNonSynv2$colMeansTsv2[PROdataNonSynv2$num%in%range]), col="blue")
  points(i, median(PROdataNonSynv3$colMeansTsv3[PROdataNonSynv3$num%in%range]), col="red")
    }
