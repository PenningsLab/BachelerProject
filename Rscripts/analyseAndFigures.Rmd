...
#KT Nov 11 2015:  I included colMeansTsv2,colMeansTsv3 in OverviewDF and OverviewDFOrderedByFreq (previously x and newdata)
...
x_tv<-data.frame(num=1:984,colMeansTv,TypeOfSite_tv)
...
```{r}
#PSP Nov 11 2015: I am not sure the next lines are correct. 

tv<-newdata_tv[order(newdata_tv[,1]),]
ts<-OverviewDFOrderedByFreq[order(OverviewDFOrderedByFreq[,1]),]
aa<-cbind(tv,ts,round(tv$colMeansTv/ts$colMeansTs,2));
...
plot(PROdata$colMeansTsv2+0.001,log="y",ylim=c(0.001,0.15),cex=2, pch = "|",col=PROdata$color)
dev.off()
...
plot(PROdata$colMeansTsv3+0.001,log="y",ylim=c(0.001,0.15),cex=2, pch = "|",col=PROdata$color)
dev.off()
```
...
# Ordered by position
#PSP Nov 11 2015 : please don't hard code the number of the column! 
names(PROdata_tv)[which(names(PROdata_tv) == "color")]<-'colortv'
